...
2. **Router Gating Sensitivity & Input Masking Confound**:
   * While raw router flips under INT4 RTN measured **$8.88\%$ for dLLM vs $4.46\%$ for AR**, this initial comparison carries a critical input confound: the AR router evaluated clean text, whereas the dLLM router evaluated 35%-masked text. A higher flip rate when routing on partially masked input is expected from input distribution shift rather than intrinsic bidirectionality. Furthermore, on an undertrained model where an 8% weight perturbation moves loss by only 8 ppm, router logits sit in near-ties, which maximizes flip instability. A definitive measurement requires matched-input evaluation (feeding AR the identical masked batch) over $\ge 3,000$ steps and multiple seeds.
