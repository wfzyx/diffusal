...
Quantization and Routing Noise in Sparse Mixture-of-Experts}\thanks{Code, model checkpoints, and interactive reproduction notebooks are available at \url{https://github.com/wfzyx/diffusal}.}}
...
Crucially, we address the question of whether diffusion models' superior relative quantization tolerance holds value if diffusion initiates from lower unquantized quality per parameter: under causal-consistent block unmasking, our diffusion model initializes with \emph{exact zero-shot parity} ($100\%$ generation identity) to the unquantized autoregressive model. When subjected to extreme ternary quantization, autoregressive decoding undergoes catastrophic compounding trajectory collapse (up to $75.0\%$ token corruption), whereas diffusion canvas decoding preserves $84.4\%$ trajectory fidelity, demonstrating that error damping translates directly into superior \emph{absolute generation quality} post-quantization.
...
\paragraph{Second-order quantization (GPTQ) in diffusion.} Post-training quantization for autoregressive Transformers was revolutionized by GPTQ~\cite{gptq}, which utilizes second-order inverse Hessian curvature updates ($\mathbf{H} = 2 \mathbf{X} \mathbf{X}^\top$) to compensate for weight rounding errors layer-by-layer. A common conjecture is that second-order Hessian methods are incompatible with diffusion models because diffusion activations vary across continuous timesteps $t \in [1, T]$. However, in causal-consistent block diffusion, historical context uses standard static KV caches (enabling direct GPTQ calibration), while candidate block activations can be calibrated across diffusion steps via Timestep-Averaged Hessian Accumulation. Crucially, our empirical results demonstrate that even \emph{without} second-order compensation (under pure round-to-nearest and naive ternary scaling), the bidirectional diffusion canvas inherently suppresses quantization-induced router drift, establishing a robust floor that second-order methods can build upon.
...
\paragraph{Matched cohorts and modern evaluation prompts.} While foundational exploratory studies frequently relied on legacy word-level corpora (such as the 1993 Penn Treebank~\cite{ptb}), contemporary LLM quantization requires validation on modern subword tokenizers and multifaceted technical domains. Accordingly, our experimental cohorts span both controlled synthetic setups and real-world production models:
\begin{itemize}
...
\item \textbf{Sparse MoE Scratch Pair:} Matched Top-2 MoE models (8 experts, 3 layers, $d=128$, active parameters $\sim$1.9M) trained across \{FP32, INT4, Ternary-QAT\}.
\item \textbf{Production MoE Checkpoint:} \texttt{PrimeIntellect/qwen3-moe-tiny} (24 layers, 16 experts per layer, Top-4 routing, $\sim$75M active params/token) using the modern Qwen 151,643-token BPE vocabulary.
\item \textbf{64-Prompt Multi-Domain Statistical Grid:} $N=64$ technical prompts spanning 8 distinct contemporary domains (Distributed Systems / Consensus, General Relativity / Theoretical Physics, Real Analysis / Mathematics, Computer Architecture / Memory Physics, Macroeconomics, Molecular Biology / CRISPR, Logic, and Systems Programming).
\end{itemize}
...
By combining ternary expert compression with Semi-AR Block Diffusion ($B=32, S=6$), active weight streaming drops to \textbf{16.1\,MB} per 64 tokens. This represents a \textbf{106.7$\times$ total data movement reduction} over FP32 AR, elevating arithmetic intensity to \textbf{53.33\,FLOPs/Byte} and yielding a \textbf{4.34$\times$ real wall-clock throughput speedup} on local CPU hardware.
...
Naive Bidirectional & \emph{``The theory of general relativity explains that gravity is  00 [CJK]  1 [CJK] |Human\# \#0Human0$<$$\backslash$  [CJK] ''} \\
...
We identify the mechanism: Rotary Position Embeddings (RoPE) under bidirectional attention evaluate negative relative offsets ($i - j < 0$), an out-of-distribution regime that disperses attention entropy. By employing \textbf{Causal-Consistent Block Attention} with confidence thresholding ($\tau = 0.85$), relative positions remain non-negative ($i - j \ge 0$), achieving \textbf{100\% exact word-for-word generation fidelity} with the AR baseline zero-shot.

\subsection{Interactive Exploration via Reactive Notebooks}
To enable transparent inspection of token-level generation trajectories, attention entropy, and expert gate activation patterns, we release an interactive reactive notebook built using the open-source Marimo framework (\texttt{marimo\_moe\_diffusion.py}). Researchers can interactively toggle precision regimes (FP32, INT4, BitNet b1.58 Ternary), vary confidence thresholds $\tau$, and observe the real-time suppression of autoregressive compounding collapse on live MoE checkpoints.
...
\bibitem{ptb} M. P. Marcus, M. A. Marcinkiewicz, B. Santorini. \emph{Building a large annotated corpus of English: The Penn Treebank}. Computational Linguistics, 19(2):313--330 (1993).
