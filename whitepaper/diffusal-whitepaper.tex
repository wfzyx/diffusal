...
\paragraph{Discrete diffusion language models.} MDLM~\cite{mdlm} provides small, well-documented masked-diffusion checkpoints with matched AR and SEDD baselines trained by the same authors on the same data. LLaDA~\cite{llada} and Dream demonstrate the paradigm at 7--8B scale. DLLMQuant~\cite{dllmquant} studies post-training quantization (PTQ) of dLLMs, but at INT4-class precisions and without AR controls.

\paragraph{Remasking.} A caution from code inspection motivates a key design choice: the well-known samplers of LLaDA and Dream are \emph{commit-and-freeze} despite ``remasking'' naming---LLaDA's low-confidence mode assigns $-\infty$ confidence to unmasked positions, so a committed token can never re-enter play. Under commit-and-freeze, the feedback loop under study barely exists: committed tokens are never revisited, and the process approaches any-order autoregression. Genuine remasking exists as an inference-time retrofit: ReMDM~\cite{remdm} re-opens committed tokens on any pretrained masked-diffusion checkpoint without retraining. Our primary sampler is therefore ReMDM; commit-and-freeze is measured as a secondary condition on the same checkpoint---a two-condition design that isolates the feedback loop with weights held fixed. Sampler implementations are validated against the sampler-correctness evaluations of Tang et al.~\cite{dllmsampler}.

\paragraph{Ternary LLMs.} BitNet b1.58~\cite{bitnet} establishes both the promise (parity with FP16 from roughly 3B parameters) and the central methodological constraint: ternary PTQ fails on \emph{every} architecture, so native low-bit training is the path, and ternary-PTQ failure on a dLLM is uninformative unless it \emph{exceeds} the failure of a matched AR model quantized identically.
...
\textbf{Method.} Run FP16 baselines for both dLLM and AR control; apply naive PTQ to both at INT8, INT4, and ternary (precision pinned per result---INT4 findings do not transfer to ternary); apply a mask-aware PTQ variant inspired by DLLMQuant to the dLLM; compare degradation \emph{deltas}: $(\text{dLLM}_{q} - \text{dLLM}_{fp16})$ versus $(\text{AR}_{q} - \text{AR}_{fp16})$.
...
\textbf{The scale confound, stated up front.} BitNet's competitiveness threshold is roughly 3B parameters; at affordable scales ternary QAT underperforms FP16 for reasons unrelated to diffusion. A small ternary dLLM losing to its FP16 twin is the expected outcome under every hypothesis and discriminates nothing. The discriminating quantity is the \textbf{gap-of-gaps}:
\[
...
\bibitem{dllmquant} Y. Xu et al. DLLMQuant: Quantizing Diffusion-based Large Language Models. \emph{arXiv:2508.14090}, 2025.
