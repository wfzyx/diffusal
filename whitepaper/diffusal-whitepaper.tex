...
The mechanism that distinguishes masked text diffusion from continuous diffusion is a \textbf{discrete token-commit decision}. With deterministic argmax decoding at a fixed position and fixed commit set, a logit perturbation smaller than the gap to the runner-up does not change token identity; a larger perturbation can flip a token discretely and, under commit-and-freeze, irreversibly. This is not total error absorption: the same perturbation can change confidence, position ranking, remasking, or stochastic sampling probabilities. The program tests whether identity preservation and revision outweigh those other pathways in end quality. An exploratory native commit-and-freeze probe changed eight fixed token IDs across 96 trajectories and found 65.7\% FP16 versus 66.3\% ternary-QAT final downstream disagreement; the three-seed QAT-minus-FP16 interval was [-2.2, +3.3] percentage points. It therefore rejects any claim that commitment alone absorbs a fixed-token error, while not resolving a QAT effect or a crossover. This finding is scoped to the local monotonic sampler: DiffusionGemma instead re-noises non-accepted canvas tokens and recomputes entropy-bound acceptance each step. A custom post-hoc revisable-remasking retrofit also produced near-complete divergence, but its checkpoints were not trained under that transition rule; it is a sampler-compatibility result, not evidence about DiffusionGemma or trained revision. It predicts:
...
\item \textbf{Per-mistake fragility.} A token flip can remain influential under commit-and-freeze, as the preliminary fixed-token probe demonstrates; confidence or schedule changes can fork a trajectory before any token flip is observed.
...
\textbf{Current replicated result.} In a frozen three-seed tiny-model experiment (Wikitext103, $\sim$7M non-embedding parameters, BitNet-style weight-only QAT), the per-seed gap ratios $R=\mathrm{tax}(\mathrm{dLLM})/\mathrm{tax}(\mathrm{AR})$ were 0.888, 0.883, and 0.898. The geometric mean was 0.890 with a two-sided 95\% $t$-interval of [0.872, 0.908]. This passes the pre-registered no-extra-tax criterion (upper interval endpoint $\leq1.25$), while not passing the stricter superior-tolerance criterion (upper endpoint $<0.80$). Thus this result supports feasibility at the tested scale, not a claim that diffusion is more ternary-tolerant. Full raw metrics and checkpoint hashes are in the accompanying replication record.
